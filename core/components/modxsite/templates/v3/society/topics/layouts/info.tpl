...
<p class="date">
    <span class="login">
        <a title="" href="{$users_url}{$object.author_username}">
            {if $src}<img src="{$src}" width="24" height="24" alt=""/>{/if}
    ...
    {date('Y-m-d', $date)}
	<span class="sep"> | </span>
	<span class="comments"><a title="" href="{$object.uri}#comments"><span class="fui-chat"></span></a> {(int)$object.comments_count} </span>
	<span class="sep"> | </span>&nbsp;
    ...
    <a href="javascript:;" class="glyphicon glyphicon-thumbs-down topic_vote" rel="nofollow" vote_direction="down"></a>
    	<span title="" class="rating act {if $rating > 0}text-primary{elseif $rating < 0}text-danger{/if}">{$rating}</span>
    ...
    {if $object.createdby == $modx->user->id || $modx->hasPermission('moderate_topics')}
    	...
    	<span class="sep"> | </span>
        <a title="Редактировать топик" class="text-info" href="{$topic_edit_url}?tid={$object.id}">
            Редактировать
        </a>
    ...
    {if $modx->hasPermission('moderate_topics')}
        ...
        {if !$object.published}
            <a title="Опубликовать топик" class="text-primary" href="javascript:;">
                Опубликовать
            </a>
        {else}
            <a title="Снять с публикации топик" class="text-warning" href="javascript:;">
                Снять с публикации
            </a>
        {/if}
        <a title="Удалить топик" class="text-danger" href="javascript:;">
            Удалить
        </a>
