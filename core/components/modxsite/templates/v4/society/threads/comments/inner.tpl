    ...
    <div class="comment-body panel panel-default">
        <div class="comment-header panel-heading">
            ...
            *}
            ...
            <a href="javascript:;" class="glyphicon glyphicon-thumbs-down comment_vote" rel="nofollow" vote_direction="down"></a>
                <span title="{implode(', ',$votes)}" class="rating act {if $rating > 0}text-primary{elseif $rating < 0}text-danger{/if}">{$rating}</span>
            <a href="javascript:;" class="glyphicon glyphicon-thumbs-up comment_vote" rel="nofollow" vote_direction="up"></a>
            
            <span class="sep">|</span>
			<span class="comment-link"><a href="{$comment.resource_uri|default:$modx->resource->uri}#comment-{$comment.id}"><i class="glyphicon glyphicon-link"></i></a></span>
		</div>
            ...
            {block name=added}
                {if $canReply}
                    <div class="comment-reply" data-comment-obj="society-comment-cont" data-comment-id="{$comment.id}">
                    	<a data-comment-obj="society-comment-action-reply" class="reviews-button-small comment-button" href="javascript:void(0)"><i class="glyphicon glyphicon-pencil"></i> Ответить</a>
